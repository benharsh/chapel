...
proc file.readerHelper(param kind=iokind.dynamic, param locking=true,
  ...
  on this._home {
    ...
    if (region.hasLowBound() && region.hasHighBound()) {
      ...
      start = region.low;
      end = max(int(64)); // TODO: shouldn't this be max(region.idxType) ?
